...
int menu() {
    ...
    while (!ready) {
        ...
        try {
            ...
            cout << "You wrote " << input << "Try again!!!" << endl;
        }
    }
    return 100; // This value is just a placeholder
}
...
int main() {
    vector <Movie> mov_ob = read_movies(); // Movies vector
    auto temp = read_series(); // Temporal variable to store the tuple containing the episode, series list, and genre list vector
    vector <Episode> all_ep_obj = get<0>(temp); // Series episodes vectors
    vector <string> series_names = get<1>(temp); // Series available in catalogue vector
    vector <string> genre_list = get<2>(temp); // Genre available in catalogue vector
    int choice = menu();
    string endprogram;
